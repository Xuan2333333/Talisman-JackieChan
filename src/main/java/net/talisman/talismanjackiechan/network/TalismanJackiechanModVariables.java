...
public class TalismanJackiechanModVariables {
	...
	public static void init(FMLCommonSetupEvent event) {
		NetworkHandler.addNetworkMessage(SavedDataSyncMessage.class, SavedDataSyncMessage::buffer, SavedDataSyncMessage::new, SavedDataSyncMessage::handler);
	}
	...
	public static class EventBusVariableHandlers {
		...
		public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
			if (!event.getEntity().level().isClientSide()) {
				...
				if (mapdata != null)
					NetworkHandler.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(0, mapdata));
				if (worlddata != null)
					NetworkHandler.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(1, worlddata));
			}
		}
	...
	public static class WorldVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level level && !level.isClientSide())
				NetworkHandler.INSTANCE.send(PacketDistributor.DIMENSION.with(level::dimension), new SavedDataSyncMessage(1, this));
		}
	...
	public static class MapVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level && !world.isClientSide())
				NetworkHandler.INSTANCE.send(PacketDistributor.ALL.noArg(), new SavedDataSyncMessage(0, this));
		}
